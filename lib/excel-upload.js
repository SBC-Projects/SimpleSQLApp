...
async function parseExcelWorkbook(base64Data, fileName) {
  ...
  const columns = usedIndexes.map((sourceIndex, visibleIndex) => {
    ...
    return {
      ...
      name,
      type,
      notNull: values.some((value) => !isBlankCell(value)) && values.every((value) => !isBlankCell(value)),
      primaryKey: false,
    };
  });
...
export function registerExcelUploadRoutes(app) {
  ...
  app.post("/api/upload/import", (req, res) => {
    try {
      ...
      const insertSql = `INSERT INTO ${quoteIdentifier(safeTableName)} (${safeColumnNames
        .map(quoteIdentifier)
